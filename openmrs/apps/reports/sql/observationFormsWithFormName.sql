...
concat(pn.given_name, " ", ifnull(pn.family_name,""))         AS "Patient Name",
  ...
  SUBSTRING(o.form_namespace_and_path, POSITION("^" IN o.form_namespace_and_path)+1,POSITION("." IN o.form_namespace_and_path)-8) as "Service",
  coalesce(o.value_numeric, o.value_text, o.value_datetime, coded_scn.name, coded_fscn.name) AS "Follow Up Date",
...
WHERE o.voided is false AND obs_fscn.name IN ('Date of Next PNC Visit','Date of Next ANC Visit','Next Visit Date','Next Vaccination Date','Follow up Date')
